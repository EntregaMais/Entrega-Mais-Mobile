...
export default function CadastroDespachanteStep1({ navigation, route }: any) {
	...
	const salvarDespachante = () => {
		...
		axios.get('http://ruby.brazilsouth.cloudapp.azure.com:3000/despachante/ok', {timeout: 10000})
			.then(response => {
			if(response.status == 200){
				axios.post('http://ruby.brazilsouth.cloudapp.azure.com:3000/despachantes', body)
					.then(res => {
						console.log(body);
